...
Properties {
	...
	_OutlineNoiseCtrl("ONC{}",Vector) = (2,0.25,1,1)
	// _OutlineRand ("")
...
SubShader {
	...
	Pass {
		...
		v2f vert (appdata v) {
			...
			float2 dir = normalize(float2(cos(_UVRT*3.1415),-sin(_UVRT*3.1415)));
			
			v2f o;
			o.vertex = UnityObjectToClipPos(v.vertex);
			o.uv = worldPos.xy + worldPos.yz + mul(dir,worldPos.xz);
		...
		fixed4 frag (v2f i) : SV_Target {
			...
			float n = lerp(_NoiseCtrl.w,0,i.mcrmd.w);
